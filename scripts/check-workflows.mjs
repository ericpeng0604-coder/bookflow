...
assert.match(handoff, /node-version:\s*22/);
assert.match(readiness, /name:\s*Release Readiness/);
assert.match(readiness, /npm ci/);
assert.match(readiness, /npm run release:local/);
...
assert.match(productionMigration, /actions:\s*read/);
assert.match(productionMigration, /GH_TOKEN/);
assert.match(productionMigration, /\^\[0-9a-f\]\{40\}\$/);
...
assert.match(uptime, /git rev-parse FETCH_HEAD/);
