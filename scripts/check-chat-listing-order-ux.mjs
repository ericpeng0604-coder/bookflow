...
const checks = [
  ...
  ["chat list selection preserves page scroll", app.includes("preservePageScroll") && app.includes("restorePageScroll(preserveScroll)") && app.includes('openConversation(conversation.id, { preservePageScroll: true })')],
  ["mobile chat back clears the restore target", app.includes('className="chat-mobile-back"') && app.includes("onBack={closeConversation}") && app.includes("function closeConversation()") && app.includes("window.localStorage.removeItem(lastChatStorageKey(currentUser.id))") && css.includes(".conversation-layout.conversation-open .conversation-list { display: none; }") && css.includes(".conversation-layout:not(.conversation-open) .conversation-panel { display: none; }")],
  ["desktop chat content stays inside its grid", css.includes("grid-template-columns: minmax(0, 1fr) minmax(0, min(46%, 360px))") && css.includes(".chat-order-status > * { min-width: 0; max-width: 100%; }") && css.includes(".trade-chat-phrases-scroll { min-width: 0; max-width: 100%;")],
