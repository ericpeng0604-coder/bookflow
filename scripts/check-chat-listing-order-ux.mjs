...
const checks = [
  ...
  ["shared meetup info uses compact right-aligned details without duplicate summary", app.includes("MeetupInfoModal") && app.includes("canEditMeetupInfo") && app.includes("onEditMeetupInfo") && app.includes("request-coordination-lines") && !app.includes("chat-meetup-summary") && !app.includes("senderName(request.buyerId)} 已送出購買意願") && !app.includes(">你已送出購買意願</p>") && app.includes("update_purchase_request_coordination") && coordinationMigration.includes("Only the listing seller can edit a fixed meetup location")],
  ...
  ["transaction details overlay without chat reflow", css.includes(".chat-route-page .chat-context-card.details-open .chat-meetup-details") && css.includes("position: absolute") && css.includes("max-height: 116px")],
  ...
  ["cart icon matches the reference line cart", app.includes('className="icon-button cart-button"') && app.includes("<CartLineIcon />") && app.includes('strokeWidth="1.6"') && css.includes(".header-actions .icon-button svg") && css.includes(".header-actions .cart-line-icon") && css.includes(".header-actions .cart-button")],
