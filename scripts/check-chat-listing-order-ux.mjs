...
const checks = [
  ...
  ["mobile chat switches between list and full-screen thread", app.includes('className="chat-mobile-back"') && app.includes("onBack={() => setExpandedConversationId(null)}") && css.includes(".conversation-layout.conversation-open .conversation-list { display: none; }") && css.includes(".conversation-layout:not(.conversation-open) .conversation-panel { display: none; }")],
