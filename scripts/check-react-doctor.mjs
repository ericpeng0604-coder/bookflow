...
function runReactDoctor() {
  ...
  const isJsEntrypoint = (path) => path && /\.(?:cjs|mjs|js)$/i.test(path);
  const isPnpmEntrypoint = (path) => path && /pnpm/i.test(path);
  const isNpmEntrypoint = (path) => path && /(?:^|[\\/])npm(?:-cli)?\.(?:cjs|mjs|js)$/i.test(path);

  let command;
  let args;
  if (isJsEntrypoint(packageManagerExecPath) && isPnpmEntrypoint(packageManagerExecPath)) {
    command = process.execPath;
    args = [packageManagerExecPath, "dlx", "react-doctor@latest", ...doctorArgs];
  } else if (existsSync(bundledPnpmPath)) {
    command = process.execPath;
    args = [bundledPnpmPath, "dlx", "react-doctor@latest", ...doctorArgs];
  } else if (isJsEntrypoint(packageManagerExecPath) && isNpmEntrypoint(packageManagerExecPath)) {
    command = process.execPath;
    args = [packageManagerExecPath, "exec", "--yes", "react-doctor@latest", "--", ...doctorArgs];
  } else {
    command = process.platform === "win32" ? "pnpm.cmd" : "pnpm";
    args = ["dlx", "react-doctor@latest", ...doctorArgs];
  }
  const result = spawnSync(command, args, {
    cwd: tempRoot,
    encoding: "utf8",
