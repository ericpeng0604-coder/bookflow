...
function usage(exitCode = 0) {
  ...
  console.log("");
  console.log("Avoid `gh pr checks --watch` during Codex releases; it repeats the full table and burns context.");
...
if (!prRef || !Number.isFinite(intervalSeconds) || intervalSeconds < 5 || !Number.isFinite(timeoutSeconds) || timeoutSeconds < 30) {
  usage(1);
}

const terminalFailure = new Set(["FAILURE", "CANCELLED", "TIMED_OUT", "ACTION_REQUIRED", "STARTUP_FAILURE"]);
const pendingStatuses = new Set(["PENDING", "QUEUED", "IN_PROGRESS", "REQUESTED", "WAITING"]);
...
function summarize(pr) {
  ...
  for (const check of checks) groups[checkState(check)].push(check);
...
function printSummary(pr, groups, final = false) {
  ...
  console.log(`${prefix}: PR #${pr.number} ${pr.isDraft ? "draft" : "ready"}; merge=${pr.mergeStateStatus || "unknown"}; pass=${groups.passed.length}; pending=${groups.pending.length}; fail=${groups.failed.length}`);
  ...
  if (groups.failed.length) {
    ...
    for (const item of groups.failed) {
      console.log(`  ${item.name}${item.workflow ? ` (${item.workflow})` : ""}: ${item.conclusion || item.status}${item.url ? ` ${item.url}` : ""}`);
    }
  }
}
