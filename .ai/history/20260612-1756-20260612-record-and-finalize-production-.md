...
## Production Lifecycle Release 2026-06-13

- PR #7 was merged to `main` at
  `a138214fe89b3dfcc44f44d8994c18cba55308b3`.
- `supabase/listing-lifecycle.sql` was applied successfully to the production
  Supabase project.
- Vercel production has `CRON_SECRET`,
  `SUPABASE_SERVICE_ROLE_KEY`, `APP_URL`, and
  `EMAIL_NOTIFICATIONS_ENABLED=false`.
- The production cron endpoint rejects unauthenticated requests with `401`.
  The first authenticated run returned `200`, with zero listings archived,
  zero cleanup actions, and Email delivery disabled.
- The production homepage loaded current listing data without browser console
  errors.
- Resend remains intentionally disabled until `RESEND_API_KEY` and
  `RESEND_FROM_EMAIL` are configured and the Email switch is changed to true.
