...
- AI 備援：
  ...
  - 直接 OpenAI 金鑰使用 Responses API；Vercel AI Gateway 使用 Chat Completions 圖片輸入及相容層 JSON schema。
...
- 正式照片測試：已定位為 `Gateway 403/customer_verification_required`，需要專案擁有者完成 Vercel AI 客戶驗證；同時修正雲端失敗時錯誤本機書名仍被寫入的缺陷。
- `tsc --noEmit`：passed。
