...
- Workflow 只在 PR、每週排程或手動觸發時執行，不使用 `push` 觸發。
- 不修改部署 workflow、資料庫、應用程式程式碼或受保護 recovery 檔案。

## 已完成

- 使用 CodeQL `security-and-quality` query suite 掃描 JavaScript/TypeScript。
...
- GitHub PR 事件仍會依專案既有設定觸發既有 release checks；本次沒有修改那些 workflow。
